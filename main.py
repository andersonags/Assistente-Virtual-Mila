...
class Virtual_assit():
    ...
    def record_audio(self, ask=""):
        ...
        with sr.Microphone() as source:
            ...
            try:
                ...
                self.engine_speak('Desculpe Anderson, não entendi o que você disse. Você pode repetir?')

            except sr.RequestError:
                self.engine_speak('Desculpe Anderson, meu servidor está fora do ar') # recognizer is not connected
    ...
    def respond(self, voice_data):
        if self.there_exist(['ola', 'mila', 'hello', 'oi', 'hey mila']):
            greetigns = [f'Oi Anderson {self.person}, o que estamos fazendo hoje?',
                        'Oi Anderson, em que posso ajudá-lo?',
                        'Oi Anderson, o que você precisa?']
...
assistent = Virtual_assit('Mila', 'Anderson')
...
while True:
    ...
    if assistent.there_exist(['bye', 'goodbye', 'Até Logo', 'Fui', 'see you']):
